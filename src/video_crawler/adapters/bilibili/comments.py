...
_IDENTITY_URL = "https://api.bilibili.com/x/web-interface/view"
_PAGE_SIZE = 20
...
async def fetch_bilibili_comments(
    ...
    if aid is None:
        identity_response, _ = await fetch_page(
            _IDENTITY_URL,
            {"bvid": target.platform_video_id},
            artifact_type="comments_identity",
            metadata={"platform_video_id": target.platform_video_id},
        )
        aid = _parse_identity_aid(identity_response.body)
...
def _target_aid(target: VideoTarget) -> int | None:
...
def _parse_identity_aid(body: bytes) -> int:
    try:
        payload: Any = json.loads(body)
    except (json.JSONDecodeError, UnicodeDecodeError) as exc:
        raise UpstreamError("invalid Bilibili video identity response") from exc
    data = payload.get("data") if isinstance(payload, Mapping) else None
    aid = data.get("aid") if isinstance(data, Mapping) else None
    if isinstance(aid, int) and not isinstance(aid, bool) and aid > 0:
        return aid
    raise UpstreamError("Bilibili video identity response did not contain aid")
